...
adata_hmap.obs['sample/clone-cluster'] = (
    adata_hmap.obs['clone-cluster'].map({'1':'', '2':' '}).astype(str) + adata.obs['sample'].astype(str)
).astype('category')

adata_hmap.obs['sample/clone-cluster'] = adata_hmap.obs['sample/clone-cluster'].cat.set_categories(
    list(dict.fromkeys(adata_hmap.obs.sort_values(by=['clone-cluster', 'sample'])['sample/clone-cluster']))
)
