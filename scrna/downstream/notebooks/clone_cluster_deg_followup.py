...
# %%
project = hp.Project("scrna/downstream", "scrna.downstream", outs = "outs/deg-clone-cluster")
...
def load_degs(sample):
    return (
        pl.read_csv(project.paths.data  / 'degs' / f"deg-{sample}-clone-cluster-1-vs-2.csv")
...
adata_sub.obs['sample/clone-cluster'] = (
    adata_sub.obs['clone-cluster'].map({'1':'', '2':' '}).astype(str) + adata.obs['sample'].astype(str)
).astype('category')

adata_sub.obs['sample/clone-cluster'] = adata_sub.obs['sample/clone-cluster'].cat.set_categories(
    list(dict.fromkeys(adata_sub.obs.sort_values(by=['clone-cluster', 'sample'])['sample/clone-cluster']))
)
