...
def assign_barcode_ids(df):
    ...
    rank_col = pl.col("percent").rank(method="ordinal", descending=True).alias("rank").cast(int)
    ids = (
        df
        .sort("percent", "barcode")
    ...
    return df.with_columns(
        pl.col("barcode").replace_strict(old=ids["barcode"], new=ids["bgID"]).alias('bgID')
    )

project = hp.Project("targeted")
library = pl.read_csv(project.paths.data / 'counts-library.tsv', separator= '\t')

assign_barcode_ids(library).select('barcode','bgID').write_csv(project.paths.data / "barcode-bgID.csv")
